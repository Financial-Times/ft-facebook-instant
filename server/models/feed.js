...
const generate = (type, articleDbRecords) => {

	return Promise.all(
		Object.keys(articleDbRecords).map(fetchArticle)
	)
	.then(articles => {
		console.log(articles.map(article => ({uuid: article.id, keys: Object.keys(article), record: articleDbRecords[article.id]})))
	});
